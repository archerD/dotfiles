...
# Lightly modified from https://github.com/davatorium/rofi-scripts/blob/master/mode-scripts/rofi-calc.sh
	...
	input=$(echo "$@" | sed 's/[[:space:]]*//g')
		if [ ! -z "$(command -v bc)" ]; then # has bc package on system
			...
			# Show the result in a notification.
                        output=$(echo $input | bc -l)
                        echo "$input = $output"
                        # something about copying is weird.
                        echo "$output" | xclip -rmlastnl -selection c &>/dev/null
