{ config, pkgs, lib, ... }:
{
  # Bootloader.
  options.archerd = {
    secure-boot = lib.mkEnableOption "secure boot, needs to be setup";
  };
  config = {
    boot.loader = {
      ...
      systemd-boot = {
        enable = !config.archerd.secure-boot;
        #/*
        windows = {
          "windows" = {
            title = "Windows";
    ...
    boot.lanzaboote = {
      enable = config.archerd.secure-boot;
      # keys can be manually generated by running sudo sbctl create-keys, and placed in ${pkiBundle}
      ...
      # `sudo sbctl sign -s /boot/EFI/edk2-uefi-shell/shell.efi`
      ...
      # NOTE: this seems to not be the case anymore?
    };
